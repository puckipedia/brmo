...
public class NHRJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        while (true) {
            ...
            List<NHRVestiging> procList = entityManager.createQuery(cq).setMaxResults(100).getResultList();
            if (procList.isEmpty()) break;

            log.info(String.format("processing %d items", procList.size()));

            for (NHRVestiging process : procList) {
                ...
                try {
                    ...
                    log.info(String.format("fetching KVK nummer %s failed with unrecoverable error", process.getKvkNummer()), e);
