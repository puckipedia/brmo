...
public class NHRLoadUtils {
    ...
    public static Dataservice getDataservice(String targetLocation, boolean preprod, NHRCertificateOptions certificateOptions) throws Exception {
        ...
        trustStore.load(new FileInputStream(certificateOptions.getTruststore()), certificateOptions.getTruststorePassword().toCharArray());
        trustManagerFactory.init(trustStore);

        KeyManagerFactory keyManagerFactory = KeyManagerFactory.getInstance("SunX509");
        KeyStore keyStore = KeyStore.getInstance("PKCS12");
        keyStore.load(new FileInputStream(certificateOptions.getKeystore()), certificateOptions.getKeystorePassword().toCharArray());
